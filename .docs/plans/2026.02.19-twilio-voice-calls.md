...
Add a settings panel (tab or section) that:
- Loads current settings from `GET /api/settings`
- Displays editable fields for `MAX_CONCURRENT_SESSIONS`, `TWILIO_AUTH_TOKEN`, `TWILIO_WEBHOOK_URL`, `TWILIO_PORT`
- Saves via `POST /api/settings`
- Shows the auth token masked with a reveal toggle
