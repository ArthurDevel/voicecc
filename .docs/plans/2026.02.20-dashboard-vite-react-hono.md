...
- [x] Install new dependencies: `hono`, `@hono/node-server`, `react`, `react-dom`, `react-router-dom`, `@twilio/voice-sdk`
- [x] Install new dev dependencies: `vite`, `@vitejs/plugin-react`, `@types/react`, `@types/react-dom`
- [x] Create `services/` directory
- [x] Create `dashboard/routes/` directory
- [x] Create `dashboard/src/`, `dashboard/src/pages/`, `dashboard/src/components/` directories
- [x] Create `dashboard/vite.config.ts`
- [x] Create `dashboard/tsconfig.json` (frontend)
- [x] Create `dashboard/index.html` (Vite entry point with `<div id="root">` and `<script type="module" src="/src/main.tsx">`)
- [x] Update root `tsconfig.json` (add services, exclude dashboard/src)

### Phase 2: Extract Services ✅

- [x] Create `services/env.ts` — extract `parseEnvFile`, `writeEnvKey` from `dashboard/server.ts`, add `readEnv` and `writeEnvFile`
- [x] Create `services/ngrok.ts` — extract ngrok process state, `startNgrok`, `stopNgrok`, `pollNgrokUrl`, `checkNgrokInstalled`, `configureNgrokAuthtoken`
- [x] Create `services/twilio-manager.ts` — extract twilio process state, `startTwilioServer`, `stopTwilioServer`, `getStatus`
- [x] Create `services/device-pairing.ts` — extract pairing codes map, device tokens map, `loadDeviceTokens`, `saveDeviceTokens`, `generatePairingCode`, `validateAndConsumeCode`, `isValidDeviceToken`

### Phase 3: Hono Server and API Routes ✅

- [x] Rewrite `dashboard/server.ts` as Hono app — `createApp` mounts route groups, `startDashboard` listens on port
- [x] Create `dashboard/routes/claude-md.ts` — `GET /` reads CLAUDE.md, `POST /` writes it, `GET /status` checks user CLAUDE.md
- [x] Create `dashboard/routes/conversations.ts` — `GET /` lists sessions, `GET /:sessionId` returns messages (move JSONL parsing logic here)
- [x] Create `dashboard/routes/settings.ts` — `GET /` reads .env with masked secrets, `POST /` merges into .env
- [x] Create `dashboard/routes/voice.ts` — `POST /start` runs osascript to open Terminal
- [x] Create `dashboard/routes/ngrok.ts` — ngrok check/authtoken/status/start/stop, delegates to ngrok service
- [x] Create `dashboard/routes/twilio.ts` — twilio status/start/stop, phone numbers, webrtc setup, token, delegates to twilio-manager service
- [x] Create `dashboard/routes/webrtc.ts` — pairing code + device validation, delegates to device-pairing service
- [x] Create `dashboard/routes/mcp-servers.ts` — `GET /` runs `claude mcp list` and parses output
- [x] Update `run.ts` — import from new `dashboard/server.ts` and `services/`

### Phase 4: React Frontend ✅

- [x] Create `dashboard/src/main.tsx` — mount React app to `#root`
- [x] Create `dashboard/src/api.ts` — shared `get`/`post` fetch wrappers
- [x] Create `dashboard/src/App.tsx` — React Router with `/` and `/call` routes
- [x] Create `dashboard/src/pages/Home.tsx` — layout with sidebar + content area, active page state
- [x] Create `dashboard/src/components/Sidebar.tsx` — nav items, voice button, browser call button, conversation list
- [x] Create `dashboard/src/components/ClaudeMdEditor.tsx` — textarea with load/save/Cmd+S/dirty state
- [x] Create `dashboard/src/components/ConversationView.tsx` — chat bubble renderer
- [x] Create `dashboard/src/components/SettingsPanel.tsx` — settings form + integration buttons
- [x] Create `dashboard/src/components/McpServersPanel.tsx` — MCP server list with status dots
- [x] Create `dashboard/src/components/TwilioPanel.tsx` — modal wizard for Twilio + WebRTC setup
- [x] Create `dashboard/src/components/BrowserCallModal.tsx` — QR code + pairing code modal
- [x] Create `dashboard/src/index.css` — port ~580 lines of inline CSS from index.html + call.html
- [x] Create `dashboard/src/pages/Call.tsx` — PIN input, pairing, Twilio Voice SDK call lifecycle

### Phase 5: Build, Wire, and Clean Up ✅

- [x] Add `build:dashboard` and `dev:dashboard` scripts to package.json
- [x] Update `postinstall` to run Vite build
- [x] Build frontend and verify Hono serves it correctly
- [x] Delete `dashboard/public/index.html`
- [x] Delete `dashboard/public/call.html`
- [x] Delete `dashboard/public/` directory
