...
- **Extended thinking narration not working**: `maxThinkingTokens: 10000` is set in claude-session.ts options, and thinking block detection code is wired (content_block_start with type "thinking" yields "Thinking..." text delta), but no thinking events are observed in the stream. Needs investigation: either the SDK/CLI doesn't forward thinking blocks, or the model isn't using extended thinking for simple voice queries. Remove `maxThinkingTokens` if it turns out to add latency without benefit.
