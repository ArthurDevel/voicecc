...
### Separate DEFAULT_CONFIG Per Entry Point

`index.ts`, `twilio-server.ts`, and `browser-server.ts` each maintain their own `DEFAULT_CONFIG`. This is intentional -- each entry point has different tuning (e.g. `interruptionThresholdMs`) and each provider should own its config. Do not extract a shared config builder.
