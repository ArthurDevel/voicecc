...
### Phase 1: Server-side -- BrowserAudioAdapter + BrowserServer -- DONE

- [x] Create `sidecar/chime.ts` -- extract `decodeChimeToPcm()` from twilio-audio.ts with PID-based temp file path
- [x] Modify `sidecar/twilio-audio.ts` -- import `decodeChimeToPcm` from chime.ts, remove local copy
- [x] Create `sidecar/browser-audio.ts` -- implement `createBrowserAudioAdapter()` with AudioAdapter interface, using chime.ts, backpressure on writeSpeaker
- [x] Create `sidecar/browser-server.ts` -- HTTP server on TWILIO_PORT, WebSocket upgrade with token validation, duplicate device token rejection, session management, dashboard proxying, ping keepalive, browser-tuned DEFAULT_CONFIG (interruptionThresholdMs: 1500)
- [x] Create `services/browser-call-manager.ts` -- spawn/stop child process

### Phase 2: Dashboard API routes -- DONE

- [x] Create `dashboard/routes/browser-call.ts` -- status/start/stop endpoints (with port conflict check against Twilio server)
- [x] Modify `dashboard/routes/twilio.ts` -- add symmetric port conflict check against browser-call server in POST /start
- [x] Modify `dashboard/server.ts` -- mount browser-call routes

### Phase 3: Client-side -- AudioWorklet + Call page -- DONE

- [x] Create `dashboard/public/audio-processor.js` -- AudioWorklet processor with ring buffer
- [x] Modify `dashboard/src/pages/Call.tsx` -- replace Twilio SDK with AudioWorklet + WebSocket, add resampling, handle getUserMedia denial, call audioContext.resume()

### Phase 4: Dashboard UI updates -- DONE

- [x] Modify `dashboard/src/pages/Home.tsx` -- remove webrtcReady from TwilioStatus, add BrowserCallStatus type
- [x] Modify `dashboard/src/pages/Settings.tsx` -- remove webrtcReady from TwilioStatus init, add browser call status polling
- [x] Create `dashboard/src/components/BrowserCallSetupPanel.tsx` -- 3-step setup wizard
- [x] Modify `dashboard/src/components/Sidebar.tsx` -- use browserCallStatus for "Call via Browser" button
- [x] Modify `dashboard/src/components/Layout.tsx` -- poll browser-call status
- [x] Modify `dashboard/src/components/BrowserCallModal.tsx` -- use browser-call status for ngrok URL

### Phase 5: Remove Twilio WebRTC code -- DONE

- [x] Modify `dashboard/src/components/TwilioPanel.tsx` -- remove "webrtc" mode, keep "twilio" only
- [x] Modify `dashboard/routes/twilio.ts` -- remove /setup-webrtc, /token, webrtcReady
- [x] Modify `package.json` -- remove `@twilio/voice-sdk`

### Phase 6: Build + Verify -- DONE

- [x] Build dashboard (`npm run build:dashboard`)
- [x] Verify no TypeScript errors (all errors are pre-existing in unrelated files)
- [x] Verify no broken imports after Twilio WebRTC removal
