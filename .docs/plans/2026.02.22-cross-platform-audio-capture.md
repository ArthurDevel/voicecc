...
- [ ] Create `audio-capture-darwin.ts` by moving all macOS-specific code from `audio-capture.ts`
- [ ] Export `startCaptureDarwin`, `stopCaptureDarwin`, `interruptPlaybackDarwin`, `resumePlaybackDarwin`
- [ ] Rewrite `audio-capture.ts` as the platform dispatcher that imports from darwin module and delegates
- [ ] Keep `bufferToFloat32` and `AudioIO` type in `audio-capture.ts`
- [ ] Verify existing macOS behavior is unchanged (run `npm start`)

### Phase 2: Linux audio capture
- [ ] Create `audio-capture-linux.ts` with `parec`/`pacat` spawning logic
- [ ] Implement echo-cancel source detection via `pactl`
- [ ] Implement gated PassThrough for stable speakerInput reference with discard flag
- [ ] Implement `startCaptureLinux` with startup validation (parec produces data within timeout)
- [ ] Implement `stopCaptureLinux` killing both processes
- [ ] Implement `interruptPlaybackLinux` (set discard flag, kill pacat) and `resumePlaybackLinux` (respawn pacat, clear discard flag)
- [ ] Add error/exit handlers on both child processes
- [ ] Wire linux module into `audio-capture.ts` dispatcher via `process.platform` check

### Phase 3: Update peripheral files
- [ ] Update `local-audio.ts` playChime to branch on platform (`afplay` on macOS, `paplay` on Linux)
- [ ] Add `sidecar/assets/chime.wav` (small WAV file, ~10KB)
- [ ] Update `stt-provider.ts`: remove `requiresPlatform: "darwin"` from local Whisper metadata
- [ ] Add `"linux"` to `package.json` os array
- [ ] Update `commandExists` helper to use `command -v` instead of `which`
